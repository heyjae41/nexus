...
원본 배열이 비었거나 상태·종료일 필드가 누락되면 후보 0건으로 간주하지 않고
스키마 오류로 수집을 중단한다. 외부 ID에는 `daker:` 접두어를 붙이며, 제목·tagline·주최사·
...
첫 페이지가 비었거나 세 상태 필드가 누락되거나 값 형식이 바뀌면 스키마 오류로
중단한다. 외부 ID에는
...
4. 이번 정상 응답에서 사라졌거나 중복으로 건너뛴 같은 소스의 기존 공개 항목은
   ...
   복구한다.
5. 같은 소스의 원본 ID만 바뀌고 제목 또는 URL이 유지되면 기존 DB 행의
   `source_id`를 교체해 숨김·신규 삽입 없이 동일 항목으로 갱신한다.
